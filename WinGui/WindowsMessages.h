...
static std::map<int, std::string> windowsMessages = {
{0x0000, "WM_NULL"},
{0x0001, "WM_CREATE"},
{0x0002, "WM_DESTROY"},
{0x0003, "WM_MOVE"},
...
static std::string msgToString(int msg) {
    auto itMessageAsString = windowsMessages.find(msg);
    if (itMessageAsString == windowsMessages.end()) {
        if (msg >= 0xC000) {
            char buff[128];
            if (GetClipboardFormatNameA(msg, buff, 128) != 0) {
                return buff;
            }
            else if (GlobalGetAtomNameA(msg, buff, 128) != 0){
                return buff;
            }
            else {
                return "unknown";
        ...
        return itMessageAsString->second;
}